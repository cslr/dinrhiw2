...
namespace whiteice
{
  ...
  bool GDALogic::initialize(whiteice::hcnode<GDAParams, whiteice::math::atlas_real<float> >* node) throw()
  {
    if(node->data == 0)
      return false;
  ...
  whiteice::math::atlas_real<float>
  GDALogic::similarity(std::vector< hcnode<GDAParams, whiteice::math::atlas_real<float> >* > clusters,
		       unsigned int i, unsigned int j) throw()
  {
    ...
    whiteice::hcnode<GDAParams, whiteice::math::atlas_real<float> > merged;
    merge(clusters[i], clusters[j], merged);
    
    whiteice::math::atlas_real<float> result = 0.0f; 
  ...
  bool GDALogic::merge(whiteice::hcnode<GDAParams, whiteice::math::atlas_real<float> >* p1,
		       whiteice::hcnode<GDAParams, whiteice::math::atlas_real<float> >* p2,
		       whiteice::hcnode<GDAParams, whiteice::math::atlas_real<float> >& result) const throw()
  {
    ...
    whiteice::math::atlas_real<float> c1, c2, c3;
    
    c1 = ((float)p1->p.n)/((float)result.p.n);
    c2 = ((float)p2->p.n)/((float)result.p.n);
    c3 = (((float)(p1->p.n * p2->p.n))/(float)(result.p.n*result.p.n)); 
    
    whiteice::math::vertex< whiteice::math::atlas_real<float> > delta(p1->p.mean - p2->p.mean);
  ...
  whiteice::math::atlas_real<float> GDALogic::overlap(const GDAParams& c1, const GDAParams& c2) const throw()
  {
    // calculates pseudooverlap (logarithm of overlap between clusters)
    
    whiteice::math::matrix< whiteice::math::atlas_real<float> > A(c1.cov), B(c2.cov), C;
    A.inv(); B.inv();
    C = A + B;
    ...
    result += (c1.mean.size()/-2.0f) * 
      log(whiteice::math::atlas_real<float>(2.0f * M_PI) * c1.cov.det() * c2.cov.det() * C.det());
    ...
    whiteice::math::vertex< whiteice::math::atlas_real<float> > m;
