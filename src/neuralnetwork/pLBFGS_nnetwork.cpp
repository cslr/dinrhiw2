...
extern "C" { static void* __plbfgs_thread_init(void *optimizer_ptr); };
...
namespace whiteice
{
  ...
  template <typename T>
  void pLBFGS_nnetwork<T>::__updater_loop()
  {
...
extern "C" {
  void* __plbfgs_thread_init(void *optimizer_ptr)
  {
    pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, 0);
    
    if(optimizer_ptr)
      ((whiteice::pLBFGS_nnetwork< whiteice::math::blas_real<float> >*)optimizer_ptr)->__updater_loop();
