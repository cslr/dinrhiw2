...
double HMM::train(const std::vector<unsigned int>& observations)
{
	...
	while(!converged) // keeps calculating EM-algorithm for parameter estimation
	{
		...
		std::cout << "Data element log(probability): " << log(po).getDouble() << std::endl;
...
/**
 * generates observations from the initial visible observation
 */
bool HMM::generate(const unsigned int state,
 ...
 */
double HMM::logprobability(std::vector<unsigned int>& observations) const
{
