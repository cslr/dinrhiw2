...
namespace whiteice
{
  namespace crypto
  {
    ...
    {
    public:
      SHA(unsigned int bits) throw(std::invalid_argument);
      virtual ~SHA();
      ...
      // compares two hashes
      bool check(const unsigned char* sha1, const unsigned char* sha2) const throw();
      
      unsigned int bits() const throw();  // number of bits  in SHA
      unsigned int bytes() const throw(); // number of bytes in SHA
      
    private:
      
      bool sha1_pad(unsigned char** data, unsigned int& length) const throw();
      bool sha512_pad(unsigned char** data, unsigned int& length) const throw();
      ...
      whiteice::uint32 sha1_fun(whiteice::uint32 x,
				   whiteice::uint32 y,
				   whiteice::uint32 z,
				   unsigned int t) const throw() PURE_FUNCTION;
      
      
      whiteice::uint32 sha256_ch(whiteice::uint32 x,
				    whiteice::uint32 y,
				    whiteice::uint32 z) const throw() PURE_FUNCTION;
	...
	const throw() PURE_FUNCTION;
      ...
      void change_endianess(whiteice::uint32& x) const throw();
      void change_endianess(whiteice::uint64& x) const throw();
