...
namespace whiteice
{
  template <typename T, typename D>
    class hash_table : public container<T,T>
    {
      ...
      virtual bool insert(const T& key, D& data) throw() = 0;
      
      /* removes key (and its data) from hash */
      virtual bool remove(const T& key, D& data) throw() = 0;
      
      /* finds given data for given key from hash or
       * throws exception if key isn't in a hash.
       */
      virtual D&   search(const T& key) throw(std::invalid_argument) = 0;
      
      /* same as search() */
      virtual D& operator[](const T& key) throw(std::invalid_argument) = 0;
    };
